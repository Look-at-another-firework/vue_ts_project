<template>
    ...
    <el-table border :data="list" style="width: 100%">
      ...
      <el-table-column prop="role" label="角色">
        <template #default="{ row }">
          <el-button type="primary" v-for="i in row.role" :key="i.role" size="small">{{
            i.roleName
          }}</el-button>
        </template>
      </el-table-column>
  ...
  <el-dialog title="编辑" v-model="isShow">
    <el-form :model="active">
      <el-form-item label="姓名" label-width="50px" placeholder="请选择姓名">
...
export default defineComponent({
  setup() {
    ...
    const onSubmit = () => {
      ...
      if (data.selectData.nikeName || data.selectData.role) {
        ...
        if (data.selectData.role) {
          arr = (data.selectData.nikeName ? arr : data.listData).filter((v: any) =>
    ...
    const userChange = () => {
      const obj: any = data.listData.find((v: any) => v.id === data.active.id)
